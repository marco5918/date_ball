...
const user = {
    ...
    /**
     * 根据用户名或电话号码查找用户业务操作
     * @param {string} loginUserOrPhone 用户名或电话号码
     * @param {int} teamId 查找该用户是否已经在球队
     * @return {object | null} 查找结果
     */
    async searchUserInfoByLoginUserOrPhone(loginUserOrPhone, teamId) {
        let resultData = {};

        let meModelArr = await MeModel.findAll({
            where: {
                [or]: [
                    { 'phone': loginUserOrPhone },
                    { 'login_user': loginUserOrPhone }
                ]
            }
        })
        ...
        if (meModelArr != null && meModelArr.length > 0) {
            ...
            let playerModelArr = await PlayerModel.findAll({
                where: {
                    [and]: [
                        { 'teamId': teamId },
                        { 'meId': resultData.me.id }
                    ]
                }
            })

            if (playerModelArr != null && playerModelArr.length > 0) {
                resultData.player = playerModelArr[0]
            }
        }
